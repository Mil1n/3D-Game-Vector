...
export function validateSettings(source = {}) {
  ...
  return {
    ...
    gameplay: {
      ...
      weaponRecoil: clamp(source.gameplay?.weaponRecoil, 0, 1, d.gameplay.weaponRecoil),
      weaponSway: Math.round(clamp(source.gameplay?.weaponSway, 0, 1, d.gameplay.weaponSway) * 20) / 20,
      slideTilt: Math.round(clamp(source.gameplay?.slideTilt, 0, 1, d.gameplay.slideTilt) * 20) / 20,
      hitStop: clamp(source.gameplay?.hitStop, 0, 1, d.gameplay.hitStop),
      enemyHitReaction: clamp(source.gameplay?.enemyHitReaction, 0, 1, d.gameplay.enemyHitReaction),
